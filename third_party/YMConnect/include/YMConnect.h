...
#define YMConnect_Header_Version "1.1.10"
...
constexpr INT32 max_alarms = 4;
constexpr INT32 alarm_history_size = 100;
constexpr INT32 max_supported_axis = 8;
constexpr INT32 OK = 0x00;
...
struct PositionData
{
	...
	PositionData(CoordinateType coordType, const Figure& fig, UINT32 toolNum, UINT32 userCoordNumber, const CoordinateArray& coordinates)  noexcept :
...
struct ByteVariableData
{
	...
	ByteVariableData(UINT8 value, UINT16 variableIndex)  noexcept :
...
struct DoubleIntVariableData
{
	...
	DoubleIntVariableData(INT32 value, UINT16 variableIndex) noexcept :
		value(value), variableIndex(variableIndex) {};
...
struct RobotPositionVariableData
{
	...
	RobotPositionVariableData(const PositionData& positionData, UINT16 variableIndex) noexcept :
...
struct StationPositionVariableData
{
	...
	StationPositionVariableData(const PositionData& positionData, UINT16 variableIndex) noexcept :
		positionData(positionData), variableIndex(variableIndex) {};
...
struct Motion
{
	...
	DOUBLE64 speed{}; // Speed of the motion. Interpreted by speedType:
	// VJ -> percent (0.00 - 100.00) for JointMotion
	// V  -> mm/sec (0.0 - 1500.0) for LinearMotion/CircularMotion
	// VR -> deg/sec (0.0 - 360.0) for rotational speed on LinearMotion/CircularMotion
	SpeedType speedType{ SpeedType::V }; // The type of speed for the motion. VJ is for JointMotion, V and VR are for LinearMotion and CircularMotion.
	...
	Motion(ControlGroupId groupId, const PositionData& destinationCoords, DOUBLE64 speed, SpeedType speedType = SpeedType::V, INT32 accuracy = -1, const MotionAccelDecel& attributes = MotionAccelDecel()) noexcept
		: groupId(groupId),
		speed(speed),
		speedType(speedType),		
...
struct LinearMotion : Motion
{
	...
	/// <param name="speed">Speed for this motion target. In mm/sec (0.0 - 1500.0) for `speedType = V`, or deg/sec (0.0 - 360.0) for `speedType = VR`</param>
	...
	/// <param name="speedType">V for linear speed (mm/sec) or VR for angular speed (deg/sec). VJ is invalid for linear interpolation.</param>
	LinearMotion(ControlGroupId groupId, const PositionData& destinationCoords, DOUBLE64 speed, SpeedType speedType = SpeedType::V, INT32 accuracy = -1, const MotionAccelDecel& attributes = MotionAccelDecel())
		: Motion(groupId, destinationCoords, speed, speedType, accuracy, attributes)
	{
...
struct JointMotion : Motion
{
	...
	//If this rounding must be minimized due to application requirements, you can specify the maximum distance (xxx units) from the target point at which the robot is allowed to deviate.</param>
	/// <param name="attributes">Optional Motion attributes.</param>
	/// <param name="speedType">Ignored; JointMotion always uses VJ (percent). Kept for API symmetry.</param>
	JointMotion(ControlGroupId groupId, const PositionData& destinationCoords, DOUBLE64 speed, SpeedType speedType = SpeedType::VJ, INT32 accuracy = -1, const MotionAccelDecel& attributes = MotionAccelDecel())
		: Motion(groupId, destinationCoords, speed, speedType, accuracy, attributes)
	{
...
struct CircularMotion : Motion
{
	...
	//If this rounding must be minimized due to application requirements, you can specify the maximum distance (xxx units) from the target point at which the robot is allowed to deviate.</param>
	/// <param name="attributes">Optional Motion attributes.</param>
	/// <param name="speedType">V for linear speed (mm/sec) or VR for angular speed (deg/sec). VJ is invalid for circular interpolation.</param>
	CircularMotion(ControlGroupId groupId, const PositionData& destinationCoords, const CoordinateArray& auxCoords, DOUBLE64 speed, SpeedType speedType = SpeedType::V, INT32 accuracy = -1, const MotionAccelDecel& attributes = MotionAccelDecel())
		: Motion(groupId, destinationCoords, speed, speedType, accuracy, attributes),
		auxCoords(auxCoords)
	{
...
public:
	...
	/// <summary>
	/// Get the remaining steps left to be executed in a trajectory.
	/// </summary>
	/// <param name="stepsRemaining">Steps left to be executed.</param>
	/// <returns></returns>
	virtual StatusInfo GetRemainingStepsInTrajectory(INT32& stepsRemaining) = 0;
};
